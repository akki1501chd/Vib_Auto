...
describe('TestSuites ', function () {
  ...
  it.only('Create Task ', () => {
    cy.fixture('example.json').then((data) => {
      ...
      cy.get('[data-timestamp="1719599400000"]').click({ force: true });// Select Due Date
      cy.get(':nth-child(2) > .css-0 > .MuiFormControl-root > .MuiInputBase-root > .MuiInputAdornment-root > .MuiButtonBase-root').click()
      cy.get('[data-timestamp="1719599400000"]').click({ force : true }); //.click()
      cy.get('.MuiAutocomplete-root > .MuiFormControl-root > .MuiInputBase-root').click({ force : true });
